...
for (i in 1:length(outer)) { # !!! if some errors try different cutoffs and/or tuneLength 
  ...
  m_inner <- train(x = ds[-outer[[i]], l_r[[i]]], y = ds[-outer[[i]],]$Label, method = "pls", trControl = tc_boot, tuneLength = 5) # adjust tuneLength to your data
...
for (i in seq_along(outer$splits)) { # !!! if some errors try different tuneLength 
  ...
  # PLS (as example) see http://topepo.github.io/caret/train-models-by-tag.html for other classification algorithm
  m_inner <- train(x = trn_x, y = trn_y, method = "pls", metric=metric, trControl = tc_boot, tuneLength = 5) # adjust tuneLength to your data
...
for (i in seq_along(outer$splits)) { # !!! if some errors try different tuneLength 
  ...
  # LM (as example) see http://topepo.github.io/caret/train-models-by-tag.html for other regression algorithm
  m_inner <- train(x = trn_x, y = trn_y, method = "lm", metric=metric, trControl = tc_boot, tuneLength = 5) # adjust tuneLength to your data
  
  # predictions in held-out fold from outer loop 
  rmse <- caret::RMSE(predict(m_inner, test), test$Label) # adjust to your data
  results <- add_row(results, fold = i, par = as.numeric(m_inner$bestTune), perf_test = rmse) # adjust to your data
}
...
for (i in 1:length(outer)) { # !!! if some errors try different tuneLength 
  ...
  # predictions in held-out fold from outer loop 
  rmse <- caret::RMSE(predict(m_inner, test), test$Label) # adjust to your data
  results <- dplyr::add_row(results, fold = i, par = as.numeric(m_inner$bestTune), perf_test = rmse) # adjust to your data
}
