...
perform_tests <- function(inner_indecies,dataframe, cutoff_tt = 0.05, cutoff_fc = 1, cutoff_vip = 1){
  ...
  lim_pval <- rownames(filter(tableTop, as.numeric(adj.P.Val) <= cutoff_tt))
  ...
  fc <- rownames(filter(tableTop_log, abs(as.numeric(logFC)) >= cutoff_fc))
...
tc_boot <- trainControl(method = "boot", number = 10, savePredictions = T, classProbs = T) # Type of resampling in outer loop for machine learning, adjust to your data
l_r <- list()
results <- data.frame(fold = numeric(), par = numeric(), acc_test = numeric()) 

# Perform
for (i in 1:length(outer)) { 
  inner <- createResample(ds[-outer[[i]],]$Label, 10) # Type of resampling in inner loop, adjust to your data
  ...
  m_inner <- train(x = ds[-outer[[i]], l_r[[i]]], y = ds[-outer[[i]],]$Label, method = "pls", trControl = tc_boot, tuneLength = 5) 
...
tc_boot <- trainControl(method = "boot", number = 10, savePredictions = T, classProbs = T) # Type of resampling in inner loop, adjust to your data
...
for (i in seq_along(outer$splits)) { 
  ...
  # PLS (as example) see http://topepo.github.io/caret/train-models-by-tag.html for other classification algorithm
  m_inner <- train(x = trn_x, y = trn_y, method = "pls", metric=metric, trControl = tc_boot, tuneLength = 5) 
  
  # predictions in held-out fold from outer loop 
  acc <- confusionMatrix(predict(m_inner, test), test$Label) 
  results <- add_row(results, fold = i, par = as.numeric(m_inner$bestTune), acc_test = acc$overall[metric]) # adjust to your data
}
...
for (i in 1:length(outer)) { 
  ...
  # predictions in held-out fold from outer loop 
  acc <- confusionMatrix(predict(m_inner, test), test$Label) 
  results <- dplyr::add_row(results, fold = i, par = as.numeric(m_inner$bestTune), acc_test = acc$overall[metric]) # adjust to your data
}
...
outer <- vfold_cv(ds, v = 10, repeats = 1, strata = "Label") # Type of resampling in outer loop, adjust to your data
tc_boot <- trainControl(method = "boot", number = 10, savePredictions = T, classProbs = F) # Type of resampling in inner loop, adjust to your data
results <- tibble(fold = numeric(), par = numeric(), acc_test = numeric()) 
...
for (i in seq_along(outer$splits)) { 
  ...
  # LM (as example) see http://topepo.github.io/caret/train-models-by-tag.html for other regression algorithm
  m_inner <- train(x = trn_x, y = trn_y, method = "lm", metric=metric, trControl = tc_boot, tuneLength = 5) 
  
  # predictions in held-out fold from outer loop 
  rmse <- caret::RMSE(predict(m_inner, test), test$Label) # adjust to your data
  results <- add_row(results, fold = i, par = as.numeric(m_inner$bestTune), acc_test = rmse) # adjust to your data
}
...
for (i in 1:length(outer)) { 
  ...
  # predictions in held-out fold from outer loop 
  rmse <- caret::RMSE(predict(m_inner, test), test$Label) # adjust to your data
  results <- dplyr::add_row(results, fold = i, par = as.numeric(m_inner$bestTune), acc_test = rmse) # adjust to your data
}
...
colnames(lm_fit_pval_all_df)[2:4] <- c("Metabolite", "Sex", "Age") # adjust to your data
lm_fit_pval_all_df
...
colnames(glm_fit_pval_all_df)[2:4] <- c("Metabolite", "Sex", "Age") # adjust to your data
glm_fit_pval_all_df
...
s_quad <- itemselect(o, select.method = "linear", FDR = 0.05) # adjust to your data "quadratic", "linear", "ANOVA"
...
res <- dbscan::dbscan(mtrx1, eps = .7, minPts = 5)
...
# 17. Smilde, Age K., et al. "ANOVA-simultaneous component analysis (ASCA): a new tool for analyzing designed metabolomics data." Bioinformatics 21.13 (2005): 3043-3048.
# 18. Sanchez-Illana, Angel, et al. "Evaluation of batch effect elimination using quality control replicates in LC-MS metabolite profiling." Analytica chimica acta 1019 (2018): 38-48.
# 19. Fages, Anne, et al. "Investigating sources of variability in metabolomic data in the EPIC study: the Principal Component Partial R-square (PC-PR2) method." Metabolomics 10.6 (2014): 1074-1083.
# 20. Yi, Sangyoon, et al. "2dFDR: a new approach to confounder adjustment substantially increases detection power in omics association studies." Genome biology 22.1 (2021): 1-18.
# 21. Rodriguez-Martinez, Andrea, et al. "MWASTools: an R/bioconductor package for metabolome-wide association studies." Bioinformatics 34.5 (2018): 890-892.
# 22. Liquet, Benoit, et al. "A novel approach for biomarker selection and the integration of repeated measures experiments from two assays." BMC bioinformatics 13.1 (2012): 1-14.
# 23. Tai, Yu Chuan, and Terence P. Speed. "A multivariate empirical Bayes statistic for replicated microarray time course data." The Annals of Statistics (2006): 2387-2412.