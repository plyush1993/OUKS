...
set.seed(1234)
# library(pamr)
fit.pam <- train(Label~ ., data=ds, method="pam", metric=metric, trControl=trainControl, tuneLength = 10)

# SVM
set.seed(1234)
# library(kernlab)
fit.svm <- train(Label~ ., data=ds, method="svmRadial", metric=metric, trControl=trainControl, tuneLength = 10)

# PLS
set.seed(1234)
# library(pls)
fit.pls <- train(Label~ ., data=ds, method="pls", metric=metric, trControl=trainControl, tuneLength = 10)

# RF
set.seed(1234)
# library(randomForest)
fit.rf <- train(Label~ ., data=ds, method="rf", metric=metric, trControl=trainControl, tuneLength = 10)
...
y <- ifelse(ds$Label == "TG", 1, 0) # set class for target -> "TG"
...
# intersect
combine <- Reduce(intersect, list(roc, uvf, lim_pval, fc, vip_pls)) # or use tuple package all => (rfe, sfs, roc, fc, uvf, lm, lmm, corr, lim_pval, pval_ttest, pval_kw, ruv2_pval, vip_pls, rf_perm_f, td_fdr, corr_f, penal_f, step_f)
# all unique
combine <- unique(c(sfs, combine)) # or use tuple package all => (rfe, sfs, roc, fc, uvf, lm, lmm, corr, lim_pval, pval_ttest, pval_kw, ruv2_pval, vip_pls, rf_perm_f, corr_f, penal_f, step_f, td_fdr)
# combination
...
set.seed(1234)
fit.cl <- train(Label~., data=dsTrain, method="pls", metric=metric, trControl=trainControl, tuneLength = 10)
...
ds <- cbind(Label = ds$Label, meta, ds[,-1])
ds$Label <- as.numeric(ds$Label) # factors to numeric
...
# perform
formula <- names(ds)[-c(1:6)] # adjust to your data
...
# ROC plot
res.roc <- roc(as.numeric(predictions), as.numeric(dsValid$Label))
...
set.seed(1234)
fit.regr <- train(Label~ ., data=dsTrain, method="lm", metric=metric, trControl=trainControl, tuneLength = 10)
# model summary
summary(fit.regr)
print(fit.regr)
# make predictions on the validation dataset
...
res.perman
res.perman.ph <- pairwise.adonis(x = ds[,-1], factors = ds$Label, p.adjust.m = "BH", perm = 1000) # perform PERMANOVA with multilevel comparison
res.perman.ph
...
sum_ancova
res_ancova <- lapply(1:length(ancova), function(y) summary(glht(ancova[[y]], linfct = mcp(Class = "Tukey"))))
res_ancova
...
dat2 <- cbind(meta, ds)
rownames(lmm_fit_pval_all_df) <- colnames(dat2)[-c(1:n_meta)]
colnames(lmm_fit_pval_all_df)[2:4] <- c("Metabolite", "Sex", "Age") # adjust to your data
lmm_fit_pval_all_df
...
colnames(glmm_fit_pval_all_df)[2:5] <- c("Intercept", "Metabolite", "Sex", "Age") # adjust to your data
...
s_quad <- itemselect(o, select.method = "quadratic", FDR = 0.05) # adjust to your data "quadratic", "linear", "ANOVA"
...
s_quad1 <- itemselect(o1, select.method = "linear", FDR = 0.05) # adjust to your data "quadratic", "linear", "ANOVA"
...
# PCA
pca.res <- pca(X = profile.filtered, ncomp = 2, scale = F, center=F) # adjust to your
plotLong(pca.res, scale = F, center = F, title = "PCA longitudinal clustering")

# s-PLS
tune.spca.res <- tuneCluster.spca(X = profile.filtered, ncomp = 2, test.keepX = c(2:ncol(profile.filtered))) # adjust to your
spca.res <- spca(X = profile.filtered, ncomp = 2, keepX = tune.spca.res$choice.keepX, scale = F) # keepX = tune.spca.res$choice.keepX or c(n, m)
...
k <- length(unique(grp1)) # groups in HC
res.pca <- PCA(mtrx1, ncp = 3, graph = F)
...
c <- fviz_nbclust(mtrx1, hcut, linecolor = "red", method = "gap_stat", nboot = 100)+ # "silhouette" or "wss" or "gap_stat"
  labs(subtitle = "")
c

# "NbClust" function
nb <- NbClust(mtrx1, distance = "manhattan", diss=NULL, min.nc = 1, max.nc = 20, method = "ward.D2", index ="all") # see index in documentation
d <- fviz_nbclust(nb)
...
# 4. Lewis, Nigel Da Costa. 100 Statistical Tests in R: What to Choose, how to Easily Calculate, with Over 300 Illustrations and Examples. Heather Hills Press, 2013.
# 5. Kabacoff, Robert. R in Action. Shelter Island, NY, USA: Manning publications, 2011.
# 6. Smyth, Gordon K. "Limma: linear models for microarray data." Bioinformatics and computational biology solutions using R and Bioconductor. Springer, New York, NY, 2005. 397-420.
# 7. De Livera, Alysha M., et al. "Normalizing and integrating metabolomics data." Analytical chemistry 84.24 (2012): 10768-10776.
# 8. Tanner, Eva M., Carl-Gustaf Bornehag, and Chris Gennings. "Repeated holdout validation for weighted quantile sum regression." MethodsX 6 (2019): 2855-2860.
# 9. Wood, Simon N. Generalized additive models: an introduction with R. CRC press, 2017.
# 10. Larras, Floriane, et al. "DRomics: A turnkey tool to support the use of the dose-response framework for Omics data in ecological risk assessment." Environmental science & technology 52.24 (2018): 14461-14468.
# 11. Yao, Cong-Hui, et al. "Dose-response metabolomics to understand biochemical mechanisms and off-target drug effects with the TOXcms software." Analytical chemistry 92.2 (2019): 1856-1864.
# 12. Bodein, Antoine, et al. "timeOmics: an R package for longitudinal multi-omics data integration." Bioinformatics (2021).
# 13. Smilde, Age K., et al. "ANOVA-simultaneous component analysis (ASCA): a new tool for analyzing designed metabolomics data." Bioinformatics 21.13 (2005): 3043-3048.
...
# 15. Fages, Anne, et al. "Investigating sources of variability in metabolomic data in the EPIC study: the Principal Component Partial R-square (PC-PR2) method." Metabolomics 10.6 (2014): 1074-1083.
# 16. Yi, Sangyoon, et al. "2dFDR: a new approach to confounder adjustment substantially increases detection power in omics association studies." Genome biology 22.1 (2021): 1-18.
# 17. Rodriguez-Martinez, Andrea, et al. "MWASTools: an R/bioconductor package for metabolome-wide association studies." Bioinformatics 34.5 (2018): 890-892.
# 18. Liquet, Benoit, et al. "A novel approach for biomarker selection and the integration of repeated measures experiments from two assays." BMC bioinformatics 13.1 (2012): 1-14.
# 19. Tai, Yu Chuan, and Terence P. Speed. "A multivariate empirical Bayes statistic for replicated microarray time course data." The Annals of Statistics (2006): 2387-2412.