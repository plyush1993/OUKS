...
# 1. RSD filtering 
# 2. (Optional) Descriptive Statistics/Missing Value/Blank/D-Ratio/Pearson Correlation filtering 
...
all_meta_data_rep <- lapply(c(1:nrow(rep_id)), function(y) dplyr::filter(all_meta_data,  s_id_p2 == rep_id[y,1])[1,]) # obtain all metadata for repeats
...
dsr3 <- data.frame(t(sapply(1:nrow(rep_id), function(t) apply(dplyr::filter(dsr2[,-1], dsr2$all_meta_data.s_id_p2 == rep_id[t,1]), 2, function(x) mean(x, na.rm = T))))) # obtain mean value for all variables for all repeats
...
colnames(dsr3)[-c(1:ncol(all_meta_data_rep))] <- colnames(dsr) # names as in loaded files
...
qc_id <- grep(pattern = "QC", x = all_meta_data$n_gr_t) # find by pattern in info from rownames
dsr_qc <- dsr[qc_id,] # select only observations by pattern from peak table
b_id_qc <- md_qc$b_id
ds <- cbind(b_id_qc, dsr_qc)
colnames(ds)[1] <- "batch"
...
rownames(dsr) <- dsr[,1]
dsr <- dsr[,-1]
dsr <- dsr[,-c(4,6,7)] # adjust to your data, as in file "all_meta_data"
...
camera <- dplyr::filter(camera, camera$annot_camera.name %in% rownames(dsr_t)[-c(1:4)])
...
xma <- dplyr::filter(xma, xma$`raw data name` %in% rownames(dsr_t)[-c(1:4)])
...
rcr <- dplyr::filter(rcr, rcr$`raw data name` %in% rownames(dsr_t)[-c(1:4)])
...
mwise <- dplyr::filter(mwise, mwise$`raw name` %in% rownames(dsr_t)[-c(1:4)])
...
qc_id <- grep(pattern = "QC", x = all_meta_data$n_gr_t) # find by pattern in info from rownames
dsr_bs <- dsr[-qc_id,] # select only observations by pattern from peak table
dsr <- dsr_bs
...
cutoff <- 30000 # set
cutoff_f <- dplyr::filter(dsc, dsc$stats >= cutoff)
...
dsr_no_NA <- dsr[,colna==F] # dataset with no NA
...
# Perform
identical(length(all_meta_data$n_gr_t), nrow(dsr_dr)) # check dimensions
qc_id <- which(all_meta_data$n_gr_t == "QC") # set QC label
sample_id <- which(all_meta_data$n_gr_t != "QC") # set label for samples
d_ratio <- sapply(1:ncol(dsr_dr), function(y) sd(dsr_dr[qc_id,y], na.rm = T)/sd(dsr_dr[sample_id,y], na.rm = T)*100) # use sd() or mad() function
thresh <- 50 # set threshold
ind_dr_tr <- which(d_ratio < thresh) # index of features under threshold
length(ind_dr_tr) # number of retained features
round(length(ind_dr_tr)/ncol(dsr_dr)*100, 0) # percent of retained features
nan_l <- length(which(is.nan(ind_dr_tr))) # number of NaN (when 0 values) 

# Subset features by D-Ratio filtering
dsr_dr_tr <- dsr_dr[,ind_dr_tr]
...
# Perform
identical(length(all_meta_data$n_gr_t), nrow(dsr_bl)) # check dimensions
bl_id <- which(all_meta_data$n_gr_t == "blank") # set blank label "blank" or which(n_gr_t != "QC") if no blank samples
cond_ind <- which(all_meta_data$n_gr_t == "QC") # set label for specific group (for example -> QC) or cond_ind <- n_gr_t[-bl_id] or which(n_gr_t == "QC") or which(n_gr_t != "QC")
...
cutoff <- thresh*cond_med # calculate cutoff
ind_bl_tr <- which(bl_med > cutoff) # index of features under cutoff
...
dsr_PR <- dplyr::filter(dsr2, s_id_p2 %in% pc_f)
dsr_PR <- dsr_PR[,-1]
...
# 6. Broadhurst, David, et al. "Guidelines and considerations for the use of system suitability and quality control samples in mass spectrometry assays applied in untargeted clinical metabolomic studies." Metabolomics 14.6 (2018): 1-17.
# 7. Helmus, Rick, et al. "patRoon: open source software platform for environmental mass spectrometry based non-target screening." Journal of Cheminformatics 13.1 (2021): 1-25.