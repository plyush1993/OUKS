...
rname <- str_remove(rname, ".CDF") # remove some pattern from vendor-specific format
all_id <- as.data.frame(lapply(1:length(rname), function(y) unlist(str_split(rname[y], " ")))) # split info from rownames
b_id <- data.frame(t(all_id[3,])) # obtain batch ID (3 row) # adjust to your data
...
rownames(dsr) <- dsr[,1]
dsr <- dsr[,-1]
dsr <- dsr[,-c(4,6,7)] # adjust to your data
...
rname_all <- rownames(dsr) # obtain all info from rownames
qc_id <- grep(pattern = "QC", x = rname_all) # find by pattern in info from rownames # adjust to your data
dsr_bs <- dsr[-qc_id,] # select only observations by pattern from peak table # adjust to your data
rname <- rownames(dsr_bs) # obtain all info from rownames
dsr <- dsr_bs
...
# Perform
identical(length(n_gr_t), nrow(dsr_dr)) # check dimensions
qc_id <- which(n_gr_t == "QC") # set QC label
sample_id <- which(n_gr_t != "QC") # set label for samples
...
cond_ind <- which(n_gr_t == "QC") # set label for specific group (for example -> QC) or cond_ind <- n_gr_t[-bl_id] or which(n_gr_t == "QC") or which(n_gr_t != "QC")
