...
# Prepare data
ds_abcs <- as.data.frame(t(cbind(Group = meta$Class, ds_norm)))
ds_abcs <- as.data.frame(cbind(Name = rownames(ds_abcs), ds_abcs))

# Perform
TransformedTable = ABCtransform(ds_abcs)
...
ds_lmm_fit <- fitMet$fittedDat[,c(n_start:ncol(fitMet$fittedDat))]
...
set.seed(1234)
# library(pamr)
fit.pam <- train(Label~ ., data=ds_abv, method="pam", metric=metric, trControl=trainControl, tuneLength = 10) # adjust tuneLength to your data

# SVM
set.seed(1234)
# library(kernlab)
fit.svm <- train(Label~ ., data=ds_abv, method="svmRadial", metric=metric, trControl=trainControl, tuneLength = 10) # adjust tuneLength to your data

# PLS
set.seed(1234)
# library(pls)
fit.pls <- train(Label~ ., data=ds_abv, method="pls", metric=metric, trControl=trainControl, tuneLength = 10) # adjust tuneLength to your data

# RF
set.seed(1234)
# library(randomForest)
fit.rf <- train(Label~ ., data=ds_abv, method="rf", metric=metric, trControl=trainControl, tuneLength = 10) # adjust tuneLength to your data
...
# features
class_un <- sum(lm_pval[,"Batch"]>0.05 & lm_pval[,"Age"]>0.05 & lm_pval[,"Sex"]>0.05 & lm_pval[,"Class"]<0.05) # adjust to your data
class_un
class_er <- sum(lm_pval[,"Batch"]>0.05 & lm_pval[,"Age"]>0.05 & lm_pval[,"Sex"]>0.05) # adjust to your data
class_er
class_er2 <- sum(lm_pval[,"Batch"]<0.05 & lm_pval[,"Age"]<0.05 & lm_pval[,"Sex"]<0.05) # adjust to your data
class_er2
...
# Single RLA
RlaPlots(featuredata = log_data$featuredata, groupdata = ds_ra[,1], ylim = c(2,-2), cex.axis = 0.6, saveinteractiveplot = F, interactiveplot = F) # adjust ylim
# Multiple RLA
