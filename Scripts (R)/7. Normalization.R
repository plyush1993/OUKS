...
# load dataset
ds <- as.data.frame(fread(input = "xcms after IPO MVI QC-XGB filter repeats annot.csv", header=T))
ds <- ds[-c(1:8),]
rownames(ds) <- ds[,5]
ds <- ds[,-c(1:5)]
...
# save
fwrite(ds_norm_sc, "xcms after IPO scal.csv", row.names = T)
...
# save
fwrite(ds_norm_dc, "xcms after IPO dc.csv", row.names = T)
...
# save
fwrite(ds_norm_cs, "xcms after IPO cs.csv", row.names = T)
...
# perform
fitMet <- fitLmm(fix=c("Sex","Age","Class"), random="(1|Batch)", data=dat, start=6)
...
# perform
fmod <- "Batch+Class+Age+Sex+Order" # or "Batch+Class+Age+Sex+Order" or "Batch+Class+Age+Sex"
kmod <- "Class"
...
# features
class_un <- sum(model[,"Batch"]>0.05 & model[,"Age"]>0.05 & model[,"Sex"]>0.05 & model[,"Class"]<0.05)
class_un
class_er <- sum(model[,"Batch"]>0.05 & model[,"Age"]>0.05 & model[,"Sex"]>0.05)
class_er
class_er2 <- sum(model[,"Batch"]<0.05 & model[,"Age"]<0.05 & model[,"Sex"]<0.05)
class_er2
class <- length(rownames(model[which(model[,"Class"]<0.05),]))
class
...
lmm_fit <- lapply(n_start:ncol(dat), function(x) lmer(dat[,x] ~ Class + Sex + Age + (1|Batch), dat))
lmm_fit_coef <- lapply(1:length(lmm_fit), function(x) summary(lmm_fit[[x]])$coefficients)
lmm_fit_pval <- sapply(1:length(lmm_fit_coef), function(x) lmm_fit_coef[[x]][,5][2])
lmm_fit_pval_all_df <- as.data.frame(t(sapply(1:length(lmm_fit_coef), function(x) lmm_fit_coef[[x]][,5])))
dat2 <- cbind(meta, ds)
rownames(lmm_fit_pval_all_df) <- colnames(dat2)[-c(1:5)]
colnames(lmm_fit_pval_all_df)[2:4] <- c("Class", "Sex", "Age")
...
# load dataset
ds <- as.data.frame(fread(input = "xcms after IPO MVI QC-XGB filter repeats annot+filtr LOG.csv", header=T))
rownames(ds) <- ds[,1]
ds <- ds[,-1]
...
ds1 <- as.data.frame(fread(input = "xcms after IPO MVI QC-XGB filter repeats annot+filtr LMM adj no norm.csv", header=T))
...
# save
fwrite(dsf_c, "xcms after IPO MVI QC-XGB filter repeats annot+filtr LMM adj.csv", row.names = T)
