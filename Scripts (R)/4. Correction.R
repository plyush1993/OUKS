...
################################################## Two Features mode
QC.TF <- function(int_data, order, batch, class, qc_label, model = "bt", nbagg = 10, k = 2) {
...
# class -> sample group variable, qc_label -> label for QC samples in group, 
# model -> type of the model ("bt" for bagging tree, "dt" - decision tree, "knn" - KNN regression), k = number of neighbours in "knn" model, nbagg -> an integer giving the number of bootstrap replications in "bt" model
qc_tf <- QC.TF(int_data = f_d, order = order, batch = batch, class = class, qc_label = "QC", model = "bt", nbagg = 10, k = 2)
# save
fwrite(qc_tf, "xcms after IPO MVI QC-BT TF.csv", row.names = T)

######################### perform decision tree in two features mode
qc_tf <- QC.TF(int_data = f_d, order = order, batch = batch, class = class, qc_label = "QC", model = "dt", nbagg = 10, k = 2)
# save
fwrite(qc_tf, "xcms after IPO MVI QC-DT TF.csv", row.names = T)

######################### perform knn in two features mode
qc_tf <- QC.TF(int_data = f_d, order = order, batch = batch, class = class, qc_label = "QC", model = "knn", nbagg = 10, k = 2)
# save
fwrite(qc_tf, "xcms after IPO MVI QC-KNN TF.csv", row.names = T)
...
################################################## Two Features mode
QC.TF.GB <- function(int_data, order, batch, class, qc_label, model = "xgboost", max.depth = 2, nrounds = 100, params) {
...
# model -> type of the model ("xgboost" for xgboost, "catboost" - catboost), 
# max_depth -> maximum depth of a tree in xgboost model, nrounds	-> max number of boosting iterations in xgboost, params -> parameters for the catboost model.
qc_tf <- QC.TF.GB(int_data = f_d, order = order, batch = batch, class = class, qc_label = "QC", model = "xgboost", max.depth = 2, nrounds = 100, params = fit_params)
# save
fwrite(qc_tf, "xcms after IPO MVI QC-XGB TF.csv", row.names = T)

######################### perform catboost in two features mode
qc_tf <- QC.TF.GB(int_data = f_d, order = order, batch = batch, class = class, qc_label = "QC", model = "catboost", max.depth = 2, nrounds = 100, params = fit_params)
# save
fwrite(qc_tf, "xcms after IPO MVI QC-CTB TF.csv", row.names = T)
...
# 13. Livera, Alysha M. De, et al. "Statistical methods for handling unwanted variation in metabolomics data." Analytical chemistry 87.7 (2015): 3606-3615.
# 14. Drotleff, Bernhard, and Michael Lammerhofer. "Guidelines for selection of internal standard-based normalization strategies in untargeted lipidomic profiling by LC-HR-MS/MS." Analytical chemistry 91.15 (2019): 9836-9843.
# 15. Sanchez-Illana, Angel, et al. "Evaluation of batch effect elimination using quality control replicates in LC-MS metabolite profiling." Analytica chimica acta 1019 (2018): 38-48.
# 16. Caesar, Lindsay K., Olav M. Kvalheim, and Nadja B. Cech. "Hierarchical cluster analysis of technical replicates to identify interferents in untargeted mass spectrometry metabolomics." Analytica chimica acta 1021 (2018): 69-77.
