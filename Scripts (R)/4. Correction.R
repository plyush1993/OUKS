...
# generate group data
group <- as.character(ds$n_gr_t)
qc_id <- which(group == "QC") # should be exactly the "QC" label for the QC samples name in table (files name in table)
group[qc_id] <- "NA"
...
metNor(
  ...
  threads = 3, # adjust to yor system
  path = new.path
)
...
################################################## Single mode
QC.SM <- function(int_data, order, class, qc_label, model = "bt", nbagg = 10, k = 2) {
...
qc_sm <- QC.SM(int_data = f_d, order = order, class = class, qc_label = "QC", model = "bt", nbagg = 10, k = 2)
# save
fwrite(qc_sm, "xcms after IPO MVI QC-BT.csv", row.names = T)

######################### perform decision tree in single mode
qc_sm <- QC.SM(int_data = f_d, order = order, class = class, qc_label = "QC", model = "dt", nbagg = 10, k = 2)
# save
fwrite(qc_sm, "xcms after IPO MVI QC-DT.csv", row.names = T)

######################### perform knn in single mode
qc_sm <- QC.SM(int_data = f_d, order = order, class = class, qc_label = "QC", model = "knn", nbagg = 10, k = 2)
# save
fwrite(qc_sm, "xcms after IPO MVI QC-KNN.csv", row.names = T)

################################################## Batchwise mode
QC.BW <- function(int_data, order, class, batch, qc_label, model = "bt", nbagg = 10, k = 2) {
...
# model -> type of the model ("bt" for bagging tree, "dt" - decision tree, "knn" - KNN regression), k = number of neighbours in "knn" model, nbagg -> an integer giving the number of bootstrap replications in "bt" model
qc_bw <- QC.BW(int_data = f_d, order = order, batch = batch, class = class, qc_label = "QC", model = "bt", nbagg = 10, k = 2)
# save
fwrite(qc_bw, "xcms after IPO MVI QC-BT bw.csv", row.names = T)

######################### perform decision tree in batchwise mode
qc_bw <- QC.BW(int_data = f_d, order = order, batch = batch, class = class, qc_label = "QC", model = "dt", nbagg = 10, k = 2)
# save
fwrite(qc_bw, "xcms after IPO MVI QC-DT bw.csv", row.names = T)

######################### perform knn in batchwise mode
qc_bw <- QC.BW(int_data = f_d, order = order, batch = batch, class = class, qc_label = "QC", model = "knn", nbagg = 10, k = 2)
# save
fwrite(qc_bw, "xcms after IPO MVI QC-KNN bw.csv", row.names = T)
...
# save
fwrite(qc_sm, "xcms after IPO MVI QC-XGB.csv", row.names = T)

######################### perform catboost in single mode
qc_sm <- QC.SM.GB(int_data = f_d, order = order, class = class, qc_label = "QC", model = "catboost", max.depth = 2, nrounds = 100, params = fit_params)
# save
fwrite(qc_sm, "xcms after IPO MVI QC-CTB.csv", row.names = T)

################################################## Batchwise mode
QC.BW.GB <- function(int_data, order, class, batch, qc_label, model = "xgboost", max.depth = 2, nrounds = 100, params) {
...
# model -> type of the model ("xgboost" for xgboost, "catboost" - catboost), 
# max_depth -> maximum depth of a tree in xgboost model, nrounds	-> max number of boosting iterations in xgboost, params -> parameters for the catboost model.
qc_bw <- QC.BW.GB(int_data = f_d, order = order, class = class, batch = batch, qc_label = "QC", model = "xgboost", max.depth = 2, nrounds = 100, params = fit_params)
# save
fwrite(qc_bw, "xcms after IPO MVI QC-XGB bw.csv", row.names = T)

######################### perform catboost in batchwise mode
qc_bw <- QC.BW.GB(int_data = f_d, order = order, class = class, batch = batch, qc_label = "QC", model = "catboost", max.depth = 2, nrounds = 100, params = fit_params)
# save
fwrite(qc_bw, "xcms after IPO MVI QC-CTB bw.csv", row.names = T)
...
PcaPlots(ccmn$featuredata, sampledata$batch) # sampledata$type or sampledata$batch
...
cor_coef <- cor(ds_cor, method = "pearson")
diag(cor_coef) <- 0
...
pvca_df <- calculate_PVCA(f_d, s_d, 
                          factors_for_PVCA = c('batch', 'class'),
...
# perform
model <- MetaboliteANOVA(dat=dat, sam=s_d, model="batch") # "batch+class" or "batch+class+order" or "batch"
...
ds_ost <- ost(ds[,-c(1:7)], p.val.sig = 0.05, p.adjust = "BH")
...
# 2. Karpievitch, Yuliya V., et al. "Metabolomics data normalization with EigenMS." PloS one 9.12 (2014): e116221.
# 3. Bararpour, Nasim, et al. "DBnorm as an R package for the comparison and selection of appropriate statistical methods for batch effect correction in metabolomic studies." Scientific reports 11.1 (2021): 1-13.
# 4. Risso, Davide, et al. "Normalization of RNA-seq data using factor analysis of control genes or samples." Nature biotechnology 32.9 (2014): 896-902.
# 5. Marr, Sue, et al. "LC-MS based plant metabolic profiles of thirteen grassland species grown in diverse neighbourhoods." Scientific data 8.1 (2021): 1-12.
# 6. Kirwan, J. A., et al. "Characterising and correcting batch variation in an automated direct infusion mass spectrometry (DIMS) metabolomics workflow." Analytical and bioanalytical chemistry 405.15 (2013): 5147-5157.
# 7. Klavus, Anton, et al. ""Notame": Workflow for Non-Targeted LC-MS Metabolic Profiling." Metabolites 10.4 (2020): 135.
# 8. Luan, Hemi, et al. "statTarget: A streamlined tool for signal drift correction and interpretations of quantitative mass spectrometry-based omics data." Analytica chimica acta 1036 (2018): 66-72.
# 9. Shen, Xiaotao, et al. "Normalization and integration of large-scale metabolomics data using support vector regression." Metabolomics 12.5 (2016): 89.
# 10. Wehrens, Ron, et al. "Improved batch correction in untargeted MS-based metabolomics." Metabolomics 12.5 (2016): 88.
# 11. Brunius, Carl, Lin Shi, and Rikard Landberg. "Large-scale untargeted LC-MS metabolomics data correction using between-batch feature alignment and cluster-based within-batch signal intensity drift correction." Metabolomics 12.11 (2016): 173.
# 12. Livera, Alysha M. De, et al. "Statistical methods for handling unwanted variation in metabolomics data." Analytical chemistry 87.7 (2015): 3606-3615.
# 13. Drotleff, Bernhard, and Michael Lammerhofer. "Guidelines for selection of internal standard-based normalization strategies in untargeted lipidomic profiling by LC-HR-MS/MS." Analytical chemistry 91.15 (2019): 9836-9843.
# 14. Sanchez-Illana, Angel, et al. "Evaluation of batch effect elimination using quality control replicates in LC-MS metabolite profiling." Analytica chimica acta 1019 (2018): 38-48.
# 15. Caesar, Lindsay K., Olav M. Kvalheim, and Nadja B. Cech. "Hierarchical cluster analysis of technical replicates to identify interferents in untargeted mass spectrometry metabolomics." Analytica chimica acta 1021 (2018): 69-77.
# 16. Fages, Anne, et al. "Investigating sources of variability in metabolomic data in the EPIC study: the Principal Component Partial R-square (PC-PR2) method." Metabolomics 10.6 (2014): 1074-1083.