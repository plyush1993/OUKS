...
ds_rlc <- ds_rlc[,-c(1:2)]
rownames(ds_rlc) <- rownames(ds)
cf <- as.data.frame(ds[,-c(1:7)]*1000/as.matrix(ds_rlc))
dsrr <- ds[,-c(1:7)]
...
ds_rfc <- ds_rfc[,-c(1:2)]
rownames(ds_rfc) <- rownames(ds)
cf <- as.data.frame(ds[,-c(1:7)]*1000/as.matrix(ds_rfc))
dsrr <- ds[,-c(1:7)]
...
# generate data
dsr2 <- as.data.frame(cbind(name = rownames(ds), ds))
dsr2 <- dsr2[,c(1:4)]
dsr2 <- as.data.frame(cbind(dsr2, ds[,-c(1:7)]))
subj_id <- which(dsr2$n_gr_t == "QC")
dsr2$n_gr_t[-subj_id] <- "Subject"
train_samples <- dsr2[dsr2$n_gr_t == "QC",] # adjust to your data

# perform
tiger <- run_TIGER(test_samples = dsr2, # adjust other parameters to your data
                   train_samples = train_samples,
                   col_sampleID  = "name",     
                   col_sampleType = "n_gr_t",  
                   col_batchID = "b_id",       
                   col_order = "ro_id",   
...
# perform
model="batch" # adjust to your data
mod_f <- as.formula(paste("y", model, sep = " ~ "))
lm_fit <- lapply(1:ncol(dat), function(y) lm(mod_f, data = cbind(s_d, y = dat[,y])))
lm_pval <- as.data.frame(sapply(1:ncol(dat), function(y) anova(lm_fit[[y]])[model, "Pr(>F)"]))
anova_per <- round(sum(lm_pval<0.05)/ncol(dat)*100, 0) # or round(sum(model[,"batch"]<0.05)/nrow(model)*100, 0) # round(sum(model[,"batch"]<0.05 & model[,"class"]>0.05)/nrow(model)*100, 0) or round(length(which(model[,"batch"]<0.05 & model[,"class"]>0.05))/nrow(model)*100, 0)
anova_per
...
round(ds_ost/ncol(ds_bd[,-c(1:7)])*100,0)
...
ds_bbc <- cbind(batch, ds_bd[,-c(1:7)])
b_b_c_subsets <- lapply(1:length(unique(ds_bbc[,1])), function(y) filter(ds_bbc[,-1], ds_bbc$batch == unique(ds_bbc[,1])[y])) # list of subsets by batches for all samples
...
ost_b <- lapply(1:length(b_b_c_subsets), function(t) ost(x = b_b_c_subsets[[t]], y = ds_bd[,-c(1:7)], p.val.sig = 0.05, p.adjust = "BH"))
