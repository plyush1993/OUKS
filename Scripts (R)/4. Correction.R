...
dat <- ds
dat <- log10(dat)
idxQC <- which(meta$n_gr_t == "QC") # adjust to your QC label
...
# Parameters for RUVr
design <- model.matrix(~class) # adjust to your data
y <- DGEList(counts=t(dat), group=class) # adjust to your data
...
# generate group data
group <- as.character(meta$n_gr_t)

# generate run order data
ro <- as.numeric(meta$ro_id)

# perform
ds_rsc <- QCRSC(df = ds, order = ro, batch = batch,
             classes = group, spar = 0, minQC = 4, qc_label = "QC", log = F) # set minQC according to your data, select QC label
...
# generate run order data
order <- as.numeric(meta$ro_id)

# Sample data
s_d <- data.frame(cbind(sample = paste(1:length(meta$n_gr_t), meta$n_gr_t), batch, class, order))
rownames(s_d) <- rownames(ds)

# Feature data
f_d <- data.frame(t(ds))
f_d <- cbind(name = colnames(ds), f_d)
colnames(f_d) <- c("name", paste(1:length(meta$n_gr_t), meta$n_gr_t))
...
samPeno <- paste(datpath,'s_d.csv', sep='/')
samFile <- paste(datpath,'f_d.csv', sep='/')
shiftCor(samPeno,samFile, MLmethod = 'QCRFSC', QCspan = 0, degree = 2, imputeM = 'min', coCV = 16000, Frule = 0.001) # set coCV as max as possible, set QCspan and degree
...
# construct data table for SERRF
f_d <- ds
dsr2 <- as.data.frame(t(f_d))
Group <- as.character(meta$n_gr_t)
QC <- as.character(meta$n_gr_t)
...
openxlsx::write.xlsx(dsr_srf, file = "for SERRF.xlsx", col.names = F, row.names = F)
...
ds_srf <- ds_srf[-c(1:4), -c(1:2)]
ds_srf <- as.data.frame(t(ds_srf))
rownames(ds_srf) <- colnames(dsr2)
...
# generate data
dsr2 <- as.data.frame(cbind(name = rownames(ds), meta))
dsr2 <- dsr2[,c(1:4)]
dsr2 <- as.data.frame(cbind(dsr2, ds))
subj_id <- which(dsr2$n_gr_t == "QC") # adjust to your QC label
dsr2$n_gr_t[-subj_id] <- "Subject"
train_samples <- dsr2[dsr2$n_gr_t == "QC",] # adjust to your QC label

# perform
tiger <- run_TIGER(test_samples = dsr2, # adjust other parameters to your data
                   train_samples = train_samples,
                   col_sampleID  = "name",     
                   col_sampleType = "n_gr_t", # adjust to your data 
                   col_batchID = "b_id", # adjust to your data       
                   col_order = "ro_id", # adjust to your data  
                   parallel.cores = 3) # adjust to your PC
...
# generate group data
class <- as.character(meta$n_gr_t)
qc_id <- which(class == "QC") # adjust to your QC label
class[-qc_id] <- "Subject"

# generate run order data
injection.order <- as.numeric(meta$ro_id)
...
QC.BW <- function(int_data, order, class, batch, qc_label, model = "bt", mode = "division", nbagg = 10, k = 2) {
  ...
  wr_batch <- pblapply(1:length(unique(int_data_ro_b_qc$batch)), function(t) dplyr::filter(int_data_ro_b_qc[,-1], int_data_ro_b_qc$batch == unique(int_data_ro_b_qc$batch)[t]))
  wr_batch_a <- pblapply(1:length(unique(int_data_ro_b$batch)), function(t) dplyr::filter(int_data_ro_b[,-1], int_data_ro_b$batch == unique(int_data_ro_b$batch)[t]))
  wr_batch_an <- pblapply(1:length(unique(int_data_ro_b$batch)), function(t) dplyr::filter(int_data_ro_b[,-c(1,2)], int_data_ro_b$batch == unique(int_data_ro_b$batch)[t]))
...
QC.BW.GB <- function(int_data, order, class, batch, qc_label, model = "xgboost", mode = "division", max.depth = 2, nrounds = 100, params) {
  ...
  ro_qc <- int_data_ro[qc_id,1]
  ro <- order
  ro_qc_b <- pblapply(1:length(unique(int_data_ro_b_qc$batch)), function(t) dplyr::filter(int_data_ro_b_qc[,-1], int_data_ro_b_qc$batch == unique(int_data_ro_b_qc$batch)[t])[,1])
  ro_b <- pblapply(1:length(unique(int_data_ro_b$batch)), function(t) dplyr::filter(int_data_ro_b[,-1], int_data_ro_b$batch == unique(int_data_ro_b$batch)[t])[,1])
  wr_batch <- pblapply(1:length(unique(int_data_ro_b_qc$batch)), function(t) dplyr::filter(int_data_ro_b_qc[,-1], int_data_ro_b_qc$batch == unique(int_data_ro_b_qc$batch)[t]))
  wr_batch_a <- pblapply(1:length(unique(int_data_ro_b$batch)), function(t) dplyr::filter(int_data_ro_b[,-1], int_data_ro_b$batch == unique(int_data_ro_b$batch)[t]))
  wr_batch_an <- pblapply(1:length(unique(int_data_ro_b$batch)), function(t) dplyr::filter(int_data_ro_b[,-c(1,2)], int_data_ro_b$batch == unique(int_data_ro_b$batch)[t]))
...
featuredata <- as.data.frame(ds)
colnames(featuredata) <- colnames(dsr)
sampledata <- data.frame(type = as.factor(meta$n_gr_t), experiment = as.factor(meta$s_id_p2), batch = as.factor(batch))
...
all_id <- lapply(1:length(rname), function(y) unlist(str_split(rname[y], " "))) 
...
ds_rsd <- as.data.frame(cbind(b_id[which(n_gr_t == "QC")], ds_rsd)) # adjust to your QC label
...
colnames(ds_dr)[1] <- "batch"

# rename batch index to numeric value
ds_dr[,1] <- str_remove(ds_dr[,1], "b")
ds_dr[,1] <- as.numeric(ds_dr[,1])
ds_dr <- ds_dr[order(ds_dr$batch, decreasing = F),]

# Select batch by pattern
tn <- ds_dr$batch # batch variables
tnu <- unique(tn)

# Batch and group
bg <- as.data.frame(cbind(batch = tn, group = n_gr_t))
...
DR_by_batch <- function (x) { 
  ...
  for (i in (1:length(DR_by_batch_results))){
    DR_by_batch_results[[i]] <- data.frame(colnames(x),DR_by_batch_results[[i]])}
  for (i in (1:length(DR_by_batch_results))){
    colnames(DR_by_batch_results[[i]]) <- c("name", "DR")}
...
dr <- DR_by_batch(ds_dr1) 

# count in % by cut-off
# cutoff 1
cutoff1 <- 30 # adjust to your data
DR_by_batch_cutoff1 <- c()
for (i in (1:length(dr))){
  DR_by_batch_cutoff1[i] <- round(length(which(dr[[i]]$DR <= cutoff1))/length(dr[[1]]$DR)*100,0)}
DR_by_batch_cutoff1 <- data.frame("batch" = n,DR_by_batch_cutoff1)
colnames(DR_by_batch_cutoff1)[2] <- c(paste(cutoff1, "% DR"))

# cutoff 2 
cutoff2 <- 50 # adjust to your data
DR_by_batch_cutoff2 <- c()
for (i in (1:length(dr))){
  DR_by_batch_cutoff2[i] <- round(length(which(dr[[i]]$DR <= cutoff2))/length(dr[[1]]$DR)*100,0)}
DR_by_batch_cutoff2 <- data.frame("batch" = n,DR_by_batch_cutoff2)
colnames(DR_by_batch_cutoff2)[2] <- c(paste(cutoff2, "% DR"))
...
pca.class <- as.character(b_id[which(n_gr_t == "QC")]) # adjust to your QC label

# perform PCA
pca.ds <- PCA(ds_bd, scale.unit = T, graph = F)
df.pca.x <- as.data.frame(pca.ds$ind$coord[,1:2]) # or pca.ds$X[,1:n], where n - number of components
...
# generate group data by batch
batch <- b_id[which(n_gr_t == "QC")] # adjust to your QC label
batch <- str_remove(batch, "b")
...
# perform
gPCA_out<-gPCA.batchdetect(x=ds,batch=batch,center=F,nperm=1000)
...
# perform
model="batch" # "batch+class" or "batch+class+order" or "batch" # adjust to your data 
...
# feature data in list by batch
ds_bbc <- cbind(batch, ds_bd)
b_b_c_subsets <- lapply(1:length(unique(ds_bbc[,1])), function(y) dplyr::filter(ds_bbc[,-1], ds_bbc$batch == unique(ds_bbc[,1])[y])) # list of subsets by batches for all samples
...
names(ost_b) <- paste0("Batch ", 1:length(ost_b))
unlist(ost_b)
mean(unlist(ost_b))
...
# dataset
mtrx1 <- ds
grp1 <- meta[,3] # 3 for batch; 1 for label
...
# generate group data
class <- as.numeric(as.integer(as.factor(meta$n_gr_t)))
...
pca <-fviz_pca_ind(pca.ds, col.ind=grp1, geom = "point", 
             gradient.cols = c("yellow", "green", "darkblue" ), legend.title = "Batch", title = "") # adjust to your data
...
ggplot(cbind(pca$data, batch, class),
...
pca.ds1 <- PCA(mtrx1, scale.unit = T, graph = F)
pca_f <- fviz_pca_ind(pca.ds1,
                      geom.ind = "point", # show points only (#text for sample names)
                      col.ind = class, repel = T,
                      addEllipses = T,title = "",
                      palette = palette_pca)

ggplot(cbind(pca_f$data,batch,class), # add 2 factors vectors
       aes(x=x,y=y, shape=batch, fill = class, color = class)) + # shape and color for different levels
  geom_point(size = 2.5) + theme_minimal() +
  scale_shape_manual(values = c(1:length(unique(batch)))) + # chose shapes
...
meta2$b_id <- as.numeric(stringr::str_remove(meta2$b_id, "b"))
meta2$n_gr_t[-which(stringr::str_detect(meta2$n_gr_t, "QC"))] <- "Sample"
ds_plot <- as.data.frame(cbind(meta2, dat, ds_s))
# ds_plot <- subset(ds_plot, meta2$n_gr_t == "Sample") # subset type of samples
# meta2 <- subset(meta2, meta2$n_gr_t == "Sample") # subset type of samples
sp <- ggplot(ds_plot, aes(x=as.numeric(meta2$ro_id), y=ds_s, color=meta2$b_id, shape = meta2$n_gr_t)) +
  ...
  scale_color_gradient(low="blue", high="red", breaks = c(as.numeric(quantile(meta2$b_id))))  +  geom_smooth(method=lm) # lm or loess
...
# generate group data by batch
batch <- meta$b_id # meta$b_id or meta$b_id[which(n_gr_t == "QC")] # adjust to your data type
batch <- str_remove(batch, "b")
batch <- as.factor(as.numeric(batch))
...
rsd <- RSD_by_batch(ds_rsd1) 
rsd2 <- rsd[[length(rsd)]]
p <- ggplot(rsd2, aes(y=rsd, x = "RSD")) + geom_violin(width=1.4, color="blue", alpha=0.8)+
                                          geom_boxplot(width=0.1, color="red", alpha=0.2)+
                                          theme_bw()
p

# boxplot by RSD per batch in QCs
rsd3 <- rsd[-length(rsd)]
rsd3 <- lapply(1:length(rsd3), function(x) rsd3[[x]][,2])
rsd_df <- data.frame(Value=unlist(rsd3), Batch=paste0("Batch ", tnu))
p <- ggplot(rsd_df, aes(y=Value, x = Batch)) + geom_violin(width=1.4, color="blue", alpha=0.8)+
  geom_boxplot(width=0.1, color="red", alpha=0.2)+
  theme_bw()
p
...
# generate group data by class
class <- as.factor(meta$n_gr_t)

# Log transform
log_data <- LogTransform(featuredata = ds_bd[,-1])
# Single RLA batch
RlaPlots(featuredata = log_data$featuredata, groupdata = batch, ylim = c(2,-2), cex.axis = 0.6, saveinteractiveplot = F, interactiveplot = F) # adjust ylim
# Single RLA class
RlaPlots(featuredata = log_data$featuredata, groupdata = class, ylim = c(2,-2), cex.axis = 0.6, saveinteractiveplot = F, interactiveplot = F) # adjust ylim
...
# 10. Fan, Sili, et al. "Systematic error removal using random forest for normalizing large-scale untargeted lipidomics data." Analytical chemistry 91.5 (2019): 3590-3596.
# 11. Han, Siyu, et al. "TIGER: technical variation elimination for metabolomics data using ensemble learning architecture." Briefings in Bioinformatics (2022).
# 12. Shen, Xiaotao, et al. "Normalization and integration of large-scale metabolomics data using support vector regression." Metabolomics 12.5 (2016): 89.
# 13. Wehrens, Ron, et al. "Improved batch correction in untargeted MS-based metabolomics." Metabolomics 12.5 (2016): 88.
# 14. Brunius, Carl, Lin Shi, and Rikard Landberg. "Large-scale untargeted LC-MS metabolomics data correction using between-batch feature alignment and cluster-based within-batch signal intensity drift correction." Metabolomics 12.11 (2016): 173.
# 15. Broadhurst, David, et al. "Guidelines and considerations for the use of system suitability and quality control samples in mass spectrometry assays applied in untargeted clinical metabolomic studies." Metabolomics 14.6 (2018): 1-17.
# 16. Plyushchenko, Ivan V., et al. "Omics Untargeted Key Script: R-Based Software Toolbox for Untargeted Metabolomics with Bladder Cancer Biomarkers Discovery Case Study." Journal of Proteome Research (2021).
# 17. Livera, Alysha M. De, et al. "Statistical methods for handling unwanted variation in metabolomics data." Analytical chemistry 87.7 (2015): 3606-3615.
# 18. Drotleff, Bernhard, and Michael Lammerhofer. "Guidelines for selection of internal standard-based normalization strategies in untargeted lipidomic profiling by LC-HR-MS/MS." Analytical chemistry 91.15 (2019): 9836-9843.
...
# 20. Caesar, Lindsay K., Olav M. Kvalheim, and Nadja B. Cech. "Hierarchical cluster analysis of technical replicates to identify interferents in untargeted mass spectrometry metabolomics." Analytica chimica acta 1021 (2018): 69-77.
# 21. Fages, Anne, et al. "Investigating sources of variability in metabolomic data in the EPIC study: the Principal Component Partial R-square (PC-PR2) method." Metabolomics 10.6 (2014): 1074-1083.
