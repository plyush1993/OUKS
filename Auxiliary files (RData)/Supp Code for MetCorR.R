...
pacman::p_load("data.table", "dplyr", "stringr", "ggplot2", "ggsci")
...
# with by s term
model <- gam(log2(tic_int_data_qc) ~ s(ro_qc, by = batch_qc))
vis.gam(model, view = c("ro_qc", "batch_qc"),
...
data <- as.data.frame(cbind(Order = ro, 
                            Model_1 = predict_gam1,
                            Model_2 = predict_gam2))
...
ggplot(init_data, aes(y = Abundance, x = Order)) + 
  ...
  theme_classic(base_size = 15) + theme(legend.position = c(0.7, 0.9)) + labs(fill = "Batch")+labs(colour = "Model:") +
  geom_line(data = data, aes(x = Order, y = value, colour = variable), alpha = 1, linewidth = 1.5)+scale_color_aaas() 

ggplot(init_data, aes(y = Abundance, x = Order)) + 
  geom_point(size=3.5, color="black", aes(fill= as.factor(Batch)), alpha=0.5, shape=21, stroke=0.5) + scale_fill_viridis_d(guide = 'none')+
  theme_classic(base_size = 15) + theme(legend.position = "none") + labs(fill = "Batch")+labs(colour = "Model:") +
  geom_line(data = data, aes(x = Order, y = value, colour = variable), alpha = 1, linewidth = 1.5)+scale_color_aaas()+
  facet_grid(~variable)
#................................................................